...
| `tastebench.py` | **The two passes no single turn can do.** The taste pass: repetition mined from the activity log, the watermark, and the guard that stops a user repeating their way into her head (`--live`, 5/5). And **the asking**: when she is nudged about a stranger, the cooldown that stops it becoming an interview, and the fact that a nudge she declines costs her nothing |
| `worthbench.py` | Whether a fact is worth a row, not just true. A music ask writes **nothing**, a taste needs evidence (they said it, or a real reason), and the drop log — five kinds of junk in, five reasons out, nothing written. Partly offline. Read its docstring: this bench asserted the opposite rule for three days |
