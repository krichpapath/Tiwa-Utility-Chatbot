...
if "--live" in sys.argv:
    ...
    try:
        ...
        print(
            "PASS live search, evidence selection, stream resolution, 100 audio frames:",
            hit["title"],
            hit["id"],
        )
    finally:
    ...
    with (
        patch.object(music, "_music_queries", return_value=["Inugami Korone song"]),
        patch.object(music, "_search", return_value=entries),
        patch.object(llm, "chat", side_effect=review),
        patch.object(music, "find", return_value=dict(entries[0], url="test-stream")) as resolve,
    ):
    ...
    print(
        "PASS original intent + canonical keywords + native/romanized evidence reach selection; exact candidate resolved"
    )
