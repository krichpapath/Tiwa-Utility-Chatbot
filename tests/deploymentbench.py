...
from tiwa import gcal, llm, memory, minis, pipeline, tools, voice
import bot
...
def calendar_failures():
    ...
    for bad in ({}, [], dict(PLAN, action="move"), dict(PLAN, title=""),
                dict(PLAN, start="2026-09-10"), dict(PLAN, end="2026-09-10T14:00:00")):
        with patch.object(gcal, "_service") as service:
            assert "failed" in gcal.apply_change(bad)
            assert not service.called
    event = dict(id="event1", summary=PLAN["title"], start={"dateTime": "2026-09-10T15:00:00+07:00"})
    with patch.object(gcal, "_service") as service:
        service.return_value.events.return_value.list.return_value.execute.return_value = {"items": [event, event]}
...
def usage_guard():
    with tempfile.TemporaryDirectory(prefix="tiwa-usage-") as scratch:
        ...
        with patch.object(llm, "SPEND_FILE", ledger):
            ...
            workers = [subprocess.Popen([sys.executable, "-c", script], env=env,
                                       stdout=subprocess.DEVNULL) for _ in range(3)]
            assert all(p.wait(timeout=30) == 0 for p in workers)
            assert llm.spend() == 140
            with patch.object(llm, "DAILY_TOKENS", 100), patch.object(llm, "LOG_PROMPTS", False), \
                 patch.object(llm, "_openrouter_chat") as remote, \
                 patch.object(llm, "_ollama_chat", return_value={"content": "local", "tool_calls": []}) as local:
    ...
    print("PASS usage thread/process contention, token ceiling routing, corrupt ledger fails closed")


def graph_boundary():
    with tempfile.TemporaryDirectory(prefix="tiwa-graph-") as scratch:
        db = memory.connect(":memory:")
        memory.remember(db, "</script><script>alert(1)</script>", "likes", "QA", "<img src=x onerror=alert(1)>")
        with patch.object(memory, "connect", return_value=db), patch.object(memory, "DATA_DIR", Path(scratch)):
