...
async def main():
    ...
    class SlowSink:
        ...
        def start(self,loop):
            assert threading.get_ident()==main_thread
        def cleanup(self): cleaned.append(self)
    ...
    with patch.object(voice,'LISTEN',True),patch.object(listening,'Ears',SlowSink):
        results=await asyncio.gather(voice.listen(guild,None,loop),voice.listen(guild,None,loop),heartbeat())
    ...
    print('PASS heartbeat responsive, concurrent starts build once, leave during load, failure then retry')
async def real_start():
    ...
    async def heartbeat():
        ...
        while True:
            await asyncio.sleep(.05)
            now=loop.time(); gaps.append(now-last); last=now
    ...
    try:
        ...
        print(f'PASS real voice.listen startup {loop.time()-start:.2f}s; {len(gaps)} heartbeat ticks; max pause {max(gaps,default=0):.3f}s',flush=True)
        assert gaps and max(gaps)<1
    finally:
        for sink in attached: sink.cleanup()
        beat.cancel()
...
for node in ast.walk(source):
    if isinstance(node,ast.Call) and isinstance(node.func,ast.Attribute) and isinstance(node.func.value,ast.Name) and node.func.value.id=='voice' and node.func.attr=='listen':
        assert isinstance(parents[node],ast.Await), 'voice.listen must be awaited'

async def guarded():
    with patch.object(listening.Path,'is_file',return_value=True), patch.dict('os.environ',{'TIWA_WAKE_MODEL':'fake.onnx'}):
        try:
