...
for text in ['I said Hey Tiwa play Judas', 'โอ๊ย กำลังโดนยิง Hey Tiwa เปิดเพลง',
             'Tiwa play music', 'Hey teacher play music', 'Hey Tiwa']:
...
class Response:
    is_error = False
    def json(self):
        return {"text": "ทิวา เปิดเพลง", "usage": {"cost": .001}}
...
class Detector:
    ...
    def predict(self, audio):
        ...
        return {'hey_tiwa': .9 if level == 2000 else 0.0}
    def reset(self):
...
ears.speakers = {}
ears.threshold, ears.silence, ears.maximum = .6, .4, 3
def pcm(level):
...
for i in range(1, 21):
    assert ears.process(2, 'same name', pcm(1000), i*.08) is None
result = ears.process(2, '2', b'', 2.6)
assert result is not None and result[0] == 'same name'
assert ears.speakers[1][1].clip is None
assert np.max(result[1]) < .1  # int16 normalized to float32
...
async def failures():
    ...
    with patch.object(stt, 'transcribe', return_value='I said Hey Tiwa play music'):
        await ears.deliver(2, 'QA', frame)
    assert messages[0][0] == ('QA', 'I said Hey Tiwa play music'), 'local activation must not be vetoed by STT'
...
print('PASS: idle isolation, preroll, silence, pause, maximum, speaker isolation, model selection, WAV, invalid audio, missing detector')
...
async def queued_commands():
    ...
    transcripts = ['เฮ้ คิวอ่ะ เปิดเพลงโซอีเตอร์ให้หน่อย',
                   'unrelated speech', 'เฮ้ ทีวีหยุดเพลงให้หน่อย', 'Hey Tiwa, hello']
