...
async def offline():
    ...
    assert (
        "Mint mother of ทิวา" in block and "Mint real name May" in block and "Gateaux" not in block
    )
    ...
    with (
        ...
        patch.object(llm, "chat", side_effect=persona),
    ):
        reply = await pipeline.respond(
            db, [{"role": "user", "content": "Tycoon: " + QUESTION}], "Tycoon", QUESTION
        )
    ...
    print(
        "PASS exact Thai regression, English, speaker/relative ownership, mother, provider failure, ambiguity, 1000 unrelated families"
    )
    db.close()
...
async def live():
    ...
    for question in (QUESTION, "พ่อหนูชื่ออะไร", "What is your father's name?"):
        ...
        ok = any(n in reply.casefold() for n in ("gateaux", "กาโต", "กาโต้", "กาโตว์")) and not any(
            n in reply.casefold() for n in ("สมชาย", "somchai")
        )
        report.append(
            {
                "question": question,
                ...
                "reply": reply,
                "mode": "stored relationship answer; real dispatch",
                "passed": ok,
            }
        )
        print(json.dumps(report[-1], ensure_ascii=False), flush=True)
    Path("qa-results/memory/relationship.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    assert all(r["passed"] for r in report), (
        "Persona failed to use the recalled name; see relationship.json"
    )
    db.close()
