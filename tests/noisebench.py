...
AUDIO = [
    ...
    ("fan hum", (0.03 * np.sin(2 * np.pi * 60 * np.arange(48000 * 3) / 48000)
                 ).astype(np.float32), 48000, False),
]
...
for label, audio, rate, want in AUDIO:
    ...
    bad += heard != want
    print(f"| {label} | {level:.4f} | {'yes: ' + text[:40] if heard else 'no'} "
...
for text, want in JUNK:
    ...
    bad += got != want
    print(f"| `{text}` | {'yes' if got else 'no'} | {'yes' if want else 'no'} "
