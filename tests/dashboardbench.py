...
facts = pd.DataFrame(
    [
        (7, False, "Krich", "plays", "Fortnite", "Thai ไทย note", "today"),
...
d.memory.remember(
    ...
    "likes",
    "Mili",
    "Thai ไทย storytelling",
    category="music",
    evidence="explicit",
    source="I love Mili — ไทย",
)
d.memory.remember(
    d.DB,
    "Krich",
    ...
    category="music",
    evidence="explicit",
    source="I hate Mili now",
)
d.memory.remember(
    d.DB, d.memory.TIWA, "likes", "Gojo", "confidence", evidence="stance", source="I like Gojo"
)
d.memory.remember(
    d.DB,
    "Mint",
...
d.memory.remember(
    ...
    "Gateaux",
    evidence="reported",
    source="Krich: Tycoon is called Gateaux",
)
...
assert (
    "Krich: Tycoon is called Gateaux"
    in d.memory_detail("Facts", SimpleNamespace(row_value=list(claims.iloc[0])))[1]
)
d.DB.execute(
    "INSERT INTO episodes(user,text,ts,evidence,source) VALUES('Krich','Krich fell off the ledge',1,'explicit','I fell off the ledge')"
)
d.DB.commit()
...
episode, detail, _ = d.memory_detail(
    "Conversations", SimpleNamespace(row_value=list(episodes.iloc[0]))
)
...
print(
    "PASS current evidence/source browsing, history, CSV/JSON exports, stale deletion, episode deletion, pass names, exact recall preview/failure"
)
d.DB.close()
scratch.cleanup()
