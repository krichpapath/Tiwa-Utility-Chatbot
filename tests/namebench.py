...
def fixture():
    ...
    memory.remember(
        db, "Tycoon", "likes", "Mili", "storytelling", category="music", evidence="explicit"
    )
...
def offline():
    ...
    store(db)
    assert db.execute(
        "SELECT evidence,source FROM relations WHERE rel='also known as'"
    ).fetchone() == ("reported", "Krich: " + CLAIM)
    ...
    assert db.execute(
        "SELECT evidence,category FROM relations WHERE rel='also known as'"
    ).fetchone() == ("explicit", "interaction")
    assert "Gateaux" in recall.context(db, "Tycoon", "hello")
    db.close()
    print(
        "PASS name evidence, future recall, ownership, topic isolation, ambiguity, existing person, self-name, 1000 unrelated facts"
    )
...
async def live():
    ...
    await asyncio.to_thread(
        memory.extract, db, "Krich", CLAIM, "Got it, you mean Tycoon is called Gateaux."
    )
    facts = memory.export_all(db)
    assert any(
        r["evidence"] == "reported" and "Krich:" in r["source"] for r in facts["relations"]
    ), facts
    report = {"claim": CLAIM, "stored": facts, "conversations": []}
    for question in (
        "Who is Gateaux?",
        "What music does Gateaux like?",
        "What does Gateaux think about Gojo?",
    ):
        ...
        reply = await pipeline.say(
            db,
            [{"role": "user", "content": "Krich: " + question}],
        ...
        report["conversations"].append(
            {
                "question": question,
                "recall": recalled,
                "bytes": len(recalled.encode()),
                "reply": reply,
            }
        )
        print(json.dumps(report["conversations"][-1], ensure_ascii=False), flush=True)
    Path("qa-results/memory/name-link.json").write_text(
