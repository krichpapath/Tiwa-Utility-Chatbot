...
start = (dt.datetime.now(dt.timezone(dt.timedelta(hours=7))) + dt.timedelta(days=1)).replace(hour=18, minute=0, second=0, microsecond=0)
...
def locate(title, at):
    hits=gcal._service().events().list(calendarId='primary',q=title,
        timeMin=at.isoformat(), timeMax=(at+dt.timedelta(hours=1)).isoformat(),
...
for role in ['Visible', 'Delete me']:
    ...
    event=hits[0]
    report['events'].append({'id':event['id'],'title':title,'url':event.get('htmlLink'),'deleted':False})
    save()
    if role=='Visible':
        edited=title+' — edited successfully'
        new_start=start+dt.timedelta(minutes=30)
        result=gcal.apply_change(dict(plan,action='edit',new_title=edited,new_start=new_start.isoformat()))
        ...
        check('verify persisted edit',current['summary']==edited and gcal._aware(current['start']['dateTime'])==new_start)
        check('preserve 15 minute duration',gcal._aware(current['end']['dateTime'])-gcal._aware(current['start']['dateTime'])==dt.timedelta(minutes=15))
        report['events'][-1].update(title=edited,start=current['start']['dateTime'],url=current.get('htmlLink'))
        ...
        save()
check('invalid event rejected',gcal.apply_change(dict(action='add',title='',start=start.isoformat())).startswith('calendar change failed:'))
