...
with patch.object(llm, "chat", return_value={"content": '{"steps":[]}'}):
    ...
    for text in ("ไม่ต้องเล่นเพลงนี้", "เล่นเพลงนี้พรุ่งนี้", "ไม่รู้จักนานาฮิระ", "play Despacito later"):
        tools.new_turn()
        minis.dj(memory.connect(":memory:"), text)
        assert not tools.DJ, text
print(
    "PASS empty DJ plan still queues explicit play; negation, future plans and artist chat stay inactive"
)
