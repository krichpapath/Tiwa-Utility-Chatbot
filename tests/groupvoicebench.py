...
def fixture():
    ...
    ears.speakers = {}
    ears.threshold, ears.silence, ears.maximum = .9, .4, 3
    ...
    for tick in range(32):
        for uid in (1, 2, 3):
            ...
            if tick >= 21 and uid != 3:
                level = 0
            result = ears.process(uid, 'same name', pcm(level), tick * .08)
            if result:
                clips[uid] = result[1]
...
async def ordering():
    ...
    async def reply(name, command, **kwargs):
        ...
        active -= 1
    ears.on_text = reply
    responses = ['Hey Tiwa first', 'Hey Tiwa second', RuntimeError('provider offline'),
                 'unrelated conversation', 'Hey Tiwa third', 'Hey Tiwa fourth']
    ...
    assert replies == [('A','first'),('B','second'),('C','unrelated conversation'),('B','third'),('A','fourth')], replies
    assert errors == ['A'] and not ears.paused
    print('PASS sequential group commands, five waiting, overflow unpaid, failure/rejection recovery')
