...
def music_checks():
    assert (
        music._rank(
            "The Tender Box Spectacular Spider-Man (Main Title)",
            song("spider", "The Spectacular Spider-Man", "The Tender Box - Topic"),
        )
        >= 0
    )
    db = memory.connect(":memory:")
    for request in (
        "ที่ว่าไม่ใช่เพลงนี้ มีลิจากลิมบัสคอมพานีต่างหาก",
        "เอ้ย พี่ว่าหยุดเพลงนี้แล้วเปิดเพลงมิลิมบัสคอมพานีแทน",
    ):
        tools.new_turn()
        with patch.object(
            llm, "chat", side_effect=AssertionError("source correction needs no invented title")
        ):
            result = minis.dj(db, request)
        assert result["action"] == "play" and result["terms"] == "Mili Limbus Company"
    tools.new_turn()
    assert (
        music._rank("Hound Dog R★O★C★K★S", song("rocks", "Hound Dog - Rocks (Naruto Opening)")) >= 0
    )
    ...
    assert (
        music._rank(
            "Steven Universe songs", song("su", "Steven Universe - Love Like You (Official Audio)")
        )
        >= 0
    )
    ...
    assert (
        music._rank(
            "Warframe Red Line", song("red", "Warframe Gauss: Redline Official Music Video")
        )
        >= 0
    )
    ...
    with patch("yt_dlp.YoutubeDL", YDL):
        ...
        with patch.object(music, "_search", return_value=[official, fan]) as search:
            ...
            assert search.call_count == 1
        for bad in (
            song("long", "Mili Hero", duration=7200),
            song("stream", "Mili Hero", is_live=True),
            wrong,
        ):
...
def reply(enough=False, answer="", sources=None, query=""):
    return {
        "content": json.dumps(
            dict(enough=enough, answer=answer, sources=sources or [], query=query)
        )
    }
...
def web_checks():
    ...
    with (
        patch.object(
            ...
            "chat",
            return_value={
                "content": json.dumps({"query": "", "clarification": "Which teams or league?"})
            },
        ),
        patch.object(tools, "web_search") as search,
    ):
    ...
    calls = [
        {"content": "Fixture X memory limit"},
        reply(query="Fixture X maximum RAM site:maker.example"),
        reply(True, "Fixture X supports 96 GB.", [2]),
    ]
    with (
        patch.object(llm, "chat", side_effect=calls),
        patch.object(tools, "web_search", side_effect=[irrelevant, relevant]) as search,
    ):
    ...
    for reviews in (
        ...
        with (
            patch.object(llm, "chat", side_effect=[{"content": "fixture"}, *reviews]),
            patch.object(tools, "web_search", return_value=irrelevant) as search,
        ):
            out = minis.search(db, "fixture fact?")
            assert "Could not establish" in out["found"], out
            assert search.call_count <= 3
    with (
        patch.object(llm, "chat", side_effect=[{"content": "fixture"}, reply(True, "fake", [1])]),
        patch.object(tools, "web_search", return_value="search failed: private error"),
    ):
        out = minis.search(db, "fixture fact?")
        assert "fake" not in out["found"] and "private error" not in out["found"]
    print(
        "web: evidence refinement, provenance, bad IDs/JSON, duplicate/budget and outage guards pass"
    )
...
def mood_choice():
    tools.new_turn()
    responses = [
        {"content": json.dumps(dict(action="play", terms="study music", selection="mood"))},
    ...
    with patch.object(
        llm,
        "chat",
        return_value={"content": json.dumps(dict(action="none", terms="", selection="mood"))},
    ) as chat:
    ...
    for selection in ("named", "mood"):
        ...
        with patch.object(
            ...
            return_value={
                "content": json.dumps(
                    dict(action="play", terms="bad spelling", selection=selection)
                )
            },
        ):
    ...
    with patch.object(
        llm,
        "chat",
        return_value={"content": json.dumps(dict(action="play", terms=url, selection="named"))},
    ):
...
def discovery_checks(_plan):
    ...
    request = {"request": "เล่นเพลงของสุยเซโฮโลไลฟ์ให้หน่อย", "keywords": "Yoh Kamiyama Suisei Hololive"}
    reviews = [
        {"content": json.dumps(dict(index=-1, query="星街すいせい official music"))},
        {"content": json.dumps(dict(index=1, query=""))},
    ]
    with (
        patch("yt_dlp.YoutubeDL", YDL),
        patch.object(music, "_search", side_effect=[[wrong], [correct]]) as search,
        patch.object(llm, "chat", side_effect=reviews) as chat,
    ):
    ...
    for response in (
        "null",
        '{"index":99,"query":""}',
        '{"index":-1,"query":"Yoh Kamiyama Suisei Hololive"}',
    ):
        with (
            patch.object(music, "_search", return_value=[wrong]) as search,
            patch.object(llm, "chat", return_value={"content": response}),
        ):
            try:
                music.find(request)
    ...
    with (
        ...
        patch.object(
            ...
            "chat",
            side_effect=[
                {"content": json.dumps(dict(index=-1, query=q))}
                for q in ("second", "third", "fourth")
            ],
        ),
    ):
        try:
    ...
    with patch("yt_dlp.YoutubeDL", YDL):
        try:
            ...
            pass
    print(
        "discovery: wrong keywords refined, real candidate selected, invalid index/JSON, budget and resolved-live guards pass"
    )
