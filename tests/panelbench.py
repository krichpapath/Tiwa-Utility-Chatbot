...
memory.log_llm(
    "ollama", "qwen3", 900, 0, "[system]\nYou are ทิวา's inner thoughts", "you remember Krich"
)
memory.log_llm(
    "ollama",
    "qwen3",
...
memory.log_llm(
    "ollama", "qwen3", 350, 0, "[system]\nYou are Memory Mini, not Tiwa voice", '{"ids":[]}'
)
memory.log_llm("openrouter", "deepseek", 700, 420, "[system]\n[inner-state — background", "เปิดให้ละ")
memory.log_llm(
    "openrouter",
    "deepseek",
    500,
    300,
    "[system]\nYou are ทิวา's private memory judgment",
    '{"memories":[]}',
)
memory.log_llm(
    "openrouter",
...
check("health states itself without colour alone", health.count("aria-label=") == 6)
check("activity plot has both axes", list(dashboard.activity_df().columns) == ["hour", "replies"])
...
check(
    "passes name current routing and recall",
    all(p in passes for p in ("dispatching", "recalling", "her reply", "remembering")),
)
...
check(
    "pass chart counts and times every pass",
    list(pf.columns) == ["pass", "calls", "median s"] and len(pf) >= 3,
)
people = dashboard.people_df()
check(
    "people chart ranks by fact count",
    list(people.columns) == ["who", "facts"]
...
check(
    "facts carry their row id",
    sorted(facts["#"]) == sorted(r[0] for r in db.execute("SELECT rowid FROM relations")),
)
...
check("blank watermark hidden", len(eps) == 1 and "guitar recording" in eps["what"][0])

calls = dashboard.llm_df()
check("llm names the passes", {"thinking", "her reply", "remembering"} <= set(calls["pass"]))
...
# exports
check(
    "export memory.json", '"subject"' in Path(dashboard.export_memory()).read_text(encoding="utf-8")
)
check(
    "export memory.csv", "guitar" in Path(dashboard.export_facts()).read_text(encoding="utf-8-sig")
)
check(
    "export episodes.csv",
    "guitar recording" in Path(dashboard.export_episodes()).read_text(encoding="utf-8-sig"),
)
check("export llm.json", '"provider"' in Path(dashboard.export_llm()).read_text(encoding="utf-8"))
check(
    "export log.csv",
    "bad apple" in Path(dashboard.export_log()).read_text(encoding="utf-8-sig").lower(),
)
check(
    "csv opens in Excel as Thai",
    Path(dashboard.export_facts()).read_bytes().startswith(b"\xef\xbb\xbf"),
)

# settings write, and only the keys the page owns
vals = [
    "api" if k == "TIWA_MODE" else "0.5" if k == "TIWA_MUSIC_VOLUME" else "" for k in dashboard.KEYS
]
dashboard.save_settings(*vals)
env = dashboard.read_env()
check("saves editable keys", env["TIWA_MODE"] == "api" and env["TIWA_MUSIC_VOLUME"] == "0.5")
...
check(
    "reset needs the tick",
    refused(dashboard.reset_settings, False) and dashboard.read_env()["TIWA_MODE"] == "api",
)
dashboard.reset_settings(True)
env = dashboard.read_env()
check("reset clears settings", "TIWA_MODE" not in env and "TIWA_MUSIC_VOLUME" not in env)
check("reset keeps secrets", env["DISCORD_TOKEN"] == "secret-must-survive")
check(
    "secrets are never rendered",
    "secret-must-survive" not in dashboard.secrets_md() and "✅ set" in dashboard.secrets_md(),
)
...
check(
    "forget ticked facts, whatever the row order",
    count("relations") == 2 and "Steven" not in set(dashboard.facts_df()["about"]),
)
...
check("wipe needs the tick", refused(dashboard.wipe, "facts", False) and count("relations") == 2)
...
check("wipe whitelist holds", refused(dashboard.wipe, "entities", True) and count("entities") >= 1)
...
check("chat says what only Discord can do", "queue lofi" in reply and "lunch tomorrow" in reply)
check(
    "chat drains the pending queues",
    tools.DJ == [] and tools.PENDING_CALENDAR == [] and tools.PENDING_MUSIC is None,
)
