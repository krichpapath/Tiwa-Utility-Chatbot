...
async def on_ready():
    ...
    async for message in channel.history(limit=20, after=datetime.now(timezone.utc) - timedelta(minutes=15)):
...
async def check_message(message, source):
    ...
    result.update(status="pass" if replied else "failed", gateway_event_received=source == "gateway",
                  human_message_verified=True, message_source=source,
                  channel_id=message.channel.id, reply_delivered=replied)
...
async def main():
    try:
        ...
        await bot.client.close()
        out = Path(__file__).resolve().parents[1] / "qa-results" / "acceptance" / "discord-human.json"
