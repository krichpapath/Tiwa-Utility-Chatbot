...
def plans():
    ...
    for steps in (
        ...
        assert [q["count"] for a, q in tools.DJ if a == "play"] == [
            s["count"] for s in steps if s["action"] == "play"
        ]
    for bad in (
        None,
        [step("stop"), step("play", "x", True)],
        [step("stop"), step("play", "x", 51)],
        [step("play", "")],
        [None],
    ):
        tools.new_turn()
...
def batches():
    assert music._song_key("TheFatRat - Monody (feat. Laura Brehm)") == music._song_key(
        "TheFatRat - Monody (Audio) ft. Laura Brehm"
    )
    entries = [
        dict(id=f"video{i:06d}", title=f"TheFatRat Song {i}", duration=180) for i in range(3)
    ]
    with (
        patch.object(music, "_music_queries", return_value=["TheFatRat songs"]),
        patch.object(music, "_search", return_value=entries),
        patch.object(
            llm, "chat", return_value={"content": json.dumps(dict(indices=[2, 0, 2], query=""))}
        ) as chat,
        patch.object(music, "find", side_effect=AssertionError("must resolve at playback")),
    ):
        hits = music.find_many(
            dict(request="TheFatRat playlist", keywords="TheFatRat songs", count=5)
        )
...
async def deck():
    ...
    with (
        patch.object(music, "find_many", side_effect=find_many),
        patch.object(
            music,
            "source_for",
            side_effect=lambda hit: Obj(title=hit["title"], cleanup=lambda: None),
        ),
        patch.object(music, "VOLUME", 1.0),
    ):
        await asyncio.gather(
            submit(("play", "artist")), submit(("play", "Judas")), submit(("play", "Miku"))
        )
        ...
        await submit(("stop", ""))
        music.QUEUE[:] = [
            dict(title="Deleted", id="deleted0000", deferred=True),
        ...
        with patch.object(
            ...
            side_effect=[
                LookupError("deleted"),
                dict(title="Still available", id="available00", url="fresh"),
            ],
        ) as resolve:
        ...
        await submit(("stop", ""))
        music.QUEUE[:] = [
            dict(title="Blocked", id="blocked0000", deferred=True),
            dict(title="Do not retry", id="retry000000", deferred=True),
        ]
        with patch.object(
            music, "find", side_effect=LookupError("YouTube is rate-limiting this machine")
        ) as resolve:
            await bot.player.advance_after(channel)
        ...
        print(
            "PASS multiple removals, named/current/queued skips, missing/ambiguous titles, counted and repeated skips"
        )
