...
def fixture():
    ...
    memory.remember(
        ...
        "likes",
        "Mili",
        "storytelling in their music",
        category="music",
        evidence="explicit",
    )
    memory.remember(db, "Mint", "likes", "ABBA", category="music", evidence="explicit")
    memory.remember(
        db, "Krich", "prefers", "brief replies", category="interaction", evidence="explicit"
    )
    db.execute(
...
def pick_all(**kwargs):
    ...
    assert (
        len(json.dumps(data["candidates"], ensure_ascii=False).encode())
        <= recall.CANDIDATE_BYTES + 2
    )
...
def offline():
    ...
    with patch.object(llm, "chat", side_effect=pick_all):
        ...
        pronoun = asyncio.run(
            recall.retrieve(db, "Krich", "Do you like him?", "Krich: What about Gojo?")
        )
        assert "ทิวา likes Gojo" in pronoun, pronoun
        switch = asyncio.run(
            recall.retrieve(db, "Krich", "Pick music for me", "Krich: What about Gojo?")
        )
        assert "Gojo" not in switch, switch
        event = asyncio.run(
            recall.retrieve(db, "Krich", "Remember when I promised to carry the team?")
        )
        ...
        for i in range(2000):
            dst = db.execute(
                "INSERT INTO entities(name) VALUES(?)", (f"unrelated_track_{i}",)
            ).lastrowid
            db.execute(
                "INSERT INTO relations(src,rel,dst,note,updated_at,category) VALUES(?,?,?,?,?,?)",
        ...
        print(
            f"PASS growth: 2,000 unrelated facts added; Gojo block unchanged ({len(large.encode())} bytes); broad music={len(flooded.encode())} bytes"
        )
    ...
    with (
        patch.object(recall, "TIMEOUT", 0.001),
        patch.object(recall, "select", side_effect=lambda *a: time.sleep(0.04)),
    ):
    ...
    with (
        patch.object(llm, "chat", side_effect=pick_all),
        patch.object(minis, "dispatch", side_effect=dispatch),
        patch.object(pipeline, "say", side_effect=say),
    ):
        asyncio.run(
            pipeline.respond(
                db,
                [{"role": "user", "content": "Krich: What do you think of Gojo?"}],
                "Krich",
    ...
    print(
        "PASS relevance: ownership, Thai, music, topic switches, pronouns, episodes, malformed IDs, failure, deadline, actual persona state"
    )
    ...
    memory.remember(
        migrated, "Krich", "hates", "Mili", evidence="explicit", source="I hate Mili now"
    )
    assert migrated.execute("SELECT rel FROM memory_history").fetchone()[0] == "likes"
    assert "likes" not in memory.lookup(migrated, "Mili")
    assert any(
        "superseded" in r["text"]
        for r in recall.candidates(migrated, "Krich", "Did I like Mili before?")
    )
    ...
    assert evidence.execute("SELECT evidence,source FROM relations").fetchone() == (
        "explicit",
        "I love Mili",
    )
    memory.store_extraction(
        ...
        {
            "memories": [],
            "episode": "Krich fell off the ledge",
            "episode_quote": "I fell off the ledge",
        },
        said="Krich: hello",
    )
    ...
    memory.store_extraction(
        ...
        said="Krich: I fell off the ledge",
    )
    assert evidence.execute("SELECT evidence,source FROM episodes").fetchone() == (
        "explicit",
        "I fell off the ledge",
    )
    print(
        "PASS writes: additive migration, evidence quotes, unrelated preference-word rejection, history and deletion"
    )
...
async def live():
    ...
    for text, expected, forbidden in cases:
        ...
        assert expected in block and not any(f in block for f in forbidden), (
            text,
            block,
        ...
        reply = await pipeline.say(
            db,
            [{"role": "user", "content": "Krich: " + text}],
        ...
        judge = await asyncio.to_thread(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Evaluate a memory-grounded conversational reply. Return JSON. "
                ...
                },
                {
                    "role": "user",
                    "content": json.dumps(
                        {"message": text, "memory": block, "reply": reply}, ensure_ascii=False
                    ),
                },
            ],
            fmt={
    ...
    for said in ("play Mili", "I love Mili, their storytelling is wonderful"):
        ...
        if said.startswith("play"):
            ...
            assert any(e == "explicit" and q in said and c == "music" for _, e, q, c in facts), (
                facts
            )
    ...
    stances = list(
        written.execute("SELECT r.rel,d.name FROM relations r JOIN entities d ON d.id=r.dst")
    )
    ...
    output.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(
        "PASS live recall/evidence; advisory reply flags:",
        sum(not r["judge"]["ok"] for r in report),
    )
