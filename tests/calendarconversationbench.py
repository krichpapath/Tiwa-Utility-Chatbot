...
async def main():
    ...
    with (
        patch.object(gcal, "confirmation_reply") as classify,
        patch.object(gcal, "apply_change", return_value="added: Checkup") as apply,
    ):
        ...
        propose()
        apply.reset_mock()
        classify.side_effect = RuntimeError("offline")
        await bot.calendar.answer(channel, 42, "go ahead")
        apply.assert_not_called()
        assert 12 in bot.calendar.pending
    print(
        "PASS conversational approval, refusal, uncertainty, revision then approval, wrong speaker, provider failure"
    )
