...
def main():
    ...
    session = types.SimpleNamespace(ready=True, decrypt=Mock(side_effect=ValueError('UnencryptedWhenPassthroughDisabled')))
    vc = types.SimpleNamespace(_connection=types.SimpleNamespace(dave_session=session),
                               _get_id_from_ssrc=lambda _: 7, mode='fixture', secret_key=b'')
    with patch.object(vr, 'PacketRouter'), patch.object(vr, 'SinkEventRouter'), \
         patch.object(vr, 'SpeakingTimer'), patch.object(vr, 'UDPKeepAlive'), \
        ...
        assert reader.decryptor.decrypt_rtp(packet) == b'valid opus'
    pushed = []
    routing = types.SimpleNamespace(_dropped_ssrcs=set(), _lock=threading.RLock(),
...
def watchdog():
    ...
    logged = []
    live = {"on": False}
    vc = types.SimpleNamespace(is_connected=lambda: True, is_listening=lambda: live["on"],
    ...
    try:
        ...
        with patch.object(listening, "Ears", lambda *args: types.SimpleNamespace(start=lambda loop: None, cleanup=lambda: None)):
            asyncio.run(ticks(3))
