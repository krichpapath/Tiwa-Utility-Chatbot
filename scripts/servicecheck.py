...
def main():
    ...
    def discord_probe():
        ...
        with httpx.Client(
            base_url="https://discord.com/api/v10", headers=headers, timeout=20
        ) as client:
    ...
    check(
        "provider",
        lambda: llm.chat(
            messages=[{"role": "user", "content": "Reply exactly: ok"}], options={"temperature": 0}
        )["content"],
    )
    check(
        "calendar",
        lambda: (
            "available" if not (s := gcal.upcoming(1)).startswith("calendar unavailable") else s
        ),
    )
    check("music_runtime", music.ready)
    check(
        "music_search",
        lambda: {k: v for k, v in music.find("Mili Hero official").items() if k == "title"},
    )
    from tiwa import tools, memory

    check(
        "web_search",
        lambda: tools.web_search(memory.connect(":memory:"), "Python official documentation")[:400],
    )
