...
def main():
    ...
    parser.add_argument(
        "--varied", action="store_true", help="Augment speaking speed, gain and background noise"
    )
    ...
    for files, label in [(trainpos, 1), (trainneg, 0)]:
        for p in files:
            ...
            if label:
                ...
                active = np.flatnonzero(abs(a) > max(0.005, float(abs(a).max()) * 0.08))
                if len(active):
            ...
            for chunk in chunks:
                ...
                for _ in range(8):
                    ...
                    sample = rng.normal(
                        0, rng.uniform(0.0001, 0.003) if args.varied else 0.0003, 32000
                    ).astype(np.float32)
                    start = int(rng.integers(0, 32000 - len(augmented) + 1))
                    sample[start : start + len(augmented)] += augmented * rng.uniform(
                        0.25 if args.varied else 0.5, 1.2
                    )
                    examples.append((np.clip(sample, -1, 1) * 32767).astype(np.int16))
                    labels.append(label)
    for _ in range(30):
        examples.append((rng.normal(0, rng.uniform(0.0001, 0.015), 32000) * 32767).astype(np.int16))
        labels.append(0)
    ...
    classifier = LogisticRegression(C=0.01, class_weight="balanced", max_iter=1000).fit(
        scaler.transform(x), labels
    )
    w = (classifier.coef_[0] / scaler.scale_).astype(np.float32).reshape(-1, 1)
    b = (classifier.intercept_ - np.sum(classifier.coef_[0] * scaler.mean_ / scaler.scale_)).astype(
        np.float32
    )
    ...
    detector = Model(wakeword_models=[str(path)], inference_framework="onnx", vad_threshold=0.5)
    results = []
    for files, label in [(testpos, 1), (testneg, 0)]:
        for p in files:
            ...
            for i in range(0, len(a) - 1279, 1280):
                scores.append(
                    float(
                        max(detector.predict((a[i : i + 1280] * 32767).astype(np.int16)).values())
                    )
                )
            row = dict(
                file=p.name, expected=label, peak=round(max(scores), 4), detected=max(scores) >= 0.6
            )
            results.append(row)
    ...
    report = dict(
        model=str(path),
        threshold=0.6,
        train_positive=[p.name for p in trainpos],
        train_negative=[p.name for p in trainneg],
        results=results,
