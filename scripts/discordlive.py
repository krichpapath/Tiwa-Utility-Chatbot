...
async def on_ready():
    ...
    try:
        await channel.send(
            "[QA acceptance] Synthetic conversation tests starting. No calendar writes or voice calls."
        )
        author = Obj(id=0, bot=False, display_name="QA Fixture", voice=None)
        for text in (
            "hello Tiwa, please answer in English",
            "ทิวา วันนี้เป็นไงบ้าง",
            "my cousin QA Steven plays guitar",
            "what instrument does QA Steven play?",
            "from now on your favourite singer is BLACKPINK",
            "play Mili Hero",
        ):
        ...
        await channel.send(
            "[QA synthetic input] Provider outage: expected visible error, then recovery."
        )
        ...
        await channel.send(
            "[QA acceptance] Synthetic delivery checks finished. Voice and calendar writes were not exercised."
        )
        ...
        out = (
            Path(__file__).resolve().parents[1] / "qa-results" / "acceptance" / "discord-live.json"
        )
