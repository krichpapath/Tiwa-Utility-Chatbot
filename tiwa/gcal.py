...
_EVENT_FORMAT = {
    ...
    "properties": {
        ...
        "new_start": {"type": ["string", "null"], "description": "New START date and time for move/reschedule. ISO with T. Null unless changing start."},
        "new_end": {"type": ["string", "null"], "description": "New END time only when explicitly requested. Moving TO a time means new_start, NOT new_end. Otherwise null preserves duration."},
    },
...
def confirmation_reply(plan, reply):
    ...
    result = llm.chat(
        ...
        messages=[{"role": "system", "content":
            "Classify the user's answer to 'Shall I save this calendar proposal?'. "
            ...
            {"role": "user", "content": json.dumps({"proposal": plan, "reply": reply}, ensure_ascii=False)}],
        fmt={"type": "object", "properties": {"decision": {"type": "string", "enum":
             ["approve", "decline", "revise", "unclear", "unrelated"]}},
...
def validate_change(ev: dict) -> dict:
    ...
    result = dict(action=ev["action"], title=ev["title"].strip(),
    ...
    if ev["action"] == "edit":
        ...
        if "new_title" in changes:
            if not isinstance(changes["new_title"], str) or not 1 <= len(changes["new_title"].strip()) <= 200:
        ...
        if "new_start" in changes and "new_end" in changes and _aware(changes["new_end"]) <= _aware(changes["new_start"]):
...
def describe_change(ev: dict) -> str:
    ...
    if ev['action'] == 'edit':
        ...
        description += " | Changes: " + ", ".join(f"{k[4:]} = {v}" for k, v in ev.items() if k.startswith('new_'))
...
def apply_change(text) -> str:
    ...
    try:
        ...
        if ev["action"] in ("cancel", "edit"):
            hits = (
                svc.events()
                .list(calendarId=calendar_id(), q=ev["title"], singleEvents=True, maxResults=250,
            ...
            if ev["action"] == "edit":
                ...
                body = {"summary": ev.get("new_title", hit.get("summary", ev["title"])),
