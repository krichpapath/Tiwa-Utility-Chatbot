...
ROOT = Path(os.getenv("TIWA_DATA_DIR") or Path(__file__).resolve().parents[1] / "data") / "wake_recordings"
...
def metadata(label, kind, notes):
    ...
    if not label or len(label) > 100 or len(notes) > 1000 or kind not in KINDS:
        raise ValueError("Use a name (1–100 characters), a sample type, and notes under 1,000 characters")
...
def save(audio, label, kind, notes):
    ...
    if not 8000 <= rate <= 192000 or samples.ndim not in (1, 2) or not .3 <= len(samples)/rate <= 60:
        raise ValueError("Record between 0.3 and 60 seconds of audio")
    if not np.isfinite(samples).all():
        raise ValueError("Audio contains invalid samples")
    if np.issubdtype(samples.dtype, np.integer):
        samples = samples.astype(np.float32) / max(abs(np.iinfo(samples.dtype).min), np.iinfo(samples.dtype).max)
    if samples.ndim == 2:
...
def trim(ident, start, end):
    ...
    return save((rate, samples[int(start*rate):int(end*rate)]), label[:85] + ' (trimmed)', kind, notes)
