...
class Gate:
    ...
    def feed(self, frame, wake, speech):
        ...
        self.elapsed += 0.08
        self.quiet = 0.0 if speech else self.quiet + 0.08
...
class Ears(voice_recv.AudioSink):
    def __init__(self, on_text, loop):
        ...
        if not path.is_file() or path.suffix != ".onnx":
            raise ValueError(
                "set TIWA_WAKE_MODEL to a trained Hey Tiwa .onnx file; listening stays off"
            )
        try:
        ...
        try:
            ...
            raise RuntimeError(
                "install optional openwakeword dependency before enabling listening"
            ) from None
        self.threshold = number("TIWA_WAKE_THRESHOLD", 0.9, 0.01, 1)
        self.silence = number("TIWA_SILENCE_S", 1.2, 0.3, 5)
        self.maximum = number("TIWA_RECORD_MAX_S", 20, 2, 30)
        self.factory = lambda: Model(
            wakeword_models=[str(path.resolve())], inference_framework="onnx", vad_threshold=0.5
        )
    ...
    def process(self, uid, name, pcm, now):
        if uid not in self.speakers:
            ...
            self.speakers[uid] = [
                detector,
                Gate(self.silence, self.maximum),
                bytearray(),
                now,
                name,
            ]
        ...
        if now - last > 0.16:
            buf.extend(b"\0" * (min(int((now - last) / 0.08), 400) * 15360))
        state[3] = now
        buf.extend(pcm)
        while len(buf) >= 15360:  # 80 ms of Discord's stereo int16 48 kHz
            ...
            frame = (
                stereo.astype(np.float32).mean(axis=1).reshape(-1, 3).mean(axis=1).astype(np.int16)
            )
            scores = detector.predict(frame)
            triggered = max(scores.values(), default=0) >= self.threshold
            speech = max(list(detector.vad.prediction_buffer)[-3:], default=0) >= 0.5
        ...
        try:
            ...
            if command:
                ...
                await asyncio.wait_for(
                    self.on_text(name, command, activated=True, user_id=uid), timeout=90
                )
                self.failures.pop(uid, None)
            ...
            try:
                await self.failed(
                    uid,
                    name,
                    "Voice command failed. No automatic retry was made. Check the bot console.",
                )
            except Exception:
                self.paused.add(uid)
                self.resume_at[uid] = time.monotonic() + 30
                print(
                    f"[voice] cannot reach text channel; paid listening paused for speaker {uid}. Retry after 30 seconds."
                )
        finally:
            self.pending.discard(uid)
        ...
        try:
            while True:
                ...
                for uid, state in list(self.speakers.items()):
                    ...
                    elif uid not in seen and state[1].clip is not None and now - state[3] > 0.16:
                    ...
                    try:
                        ...
                        if result:
                            try:
                                ...
                                print(
                                    f"[voice] command queued for speaker {uid} ({self.commands.qsize()} waiting)"
                                )
                            except asyncio.QueueFull:
                                # No STT charge for an overflowed clip.
                                print(
                                    "[voice] command queue full; clip skipped without transcription charge"
                                )
                    except Exception as error:
